...
if ($nro_slide >= 7){
	$this->form_validation->set_rules("C24","Marca Sí o No",'required');
 }
 if ($nro_slide >= 8){
	...
	$this->form_validation->set_rules("C25k","Selecciona una opción",'callback_C25consumo');
...
if ($nro_slide >= 14){
	$this->form_validation->set_rules('D49',"Marca Sí o No",'callback_tubo_relacion['.'Marca Sí o No'.']');
		...
		if ($nro_slide >= 4){
			for($i=0; $i<14;$i++) //limpia valores
			   ...
			   if ($form_data['A8g']==1)#ERROR DETECTADO 25/08/2016 ESTABA $form_data['A8g']=1
			    	  $form_data['B15e']=9;
			   if ($form_data['A9']==2) #ERROR DETECTADO 24/08/2016 ESTABA $form_data['A8g']=2
			    	  $form_data['B15j']=9;
...
else // passed validation proceed to post success logic
{
	...
	if ($nro_slide==16){
		$mensaje = "<img src='".base_url('images/danecitos.jpg')."' class='img-responsive'>";
...
if ($opcion == 7){
	if (    $this->input->post('C25a')==7 
		...
		{
		 $this->form_validation->set_message('C25consumo', 'Nos indicaste haber consumido alguna sustancia psicoactiva, por favor revisa la respuesta a la pregunta 24.');
		    return false;
		}
		else 
			return true;
}
else if ($opcion == null and  $this->input->post('C24') == 1){
			$this->form_validation->set_message('C25consumo', 'Selecciona una opcion.');
		return false;
		}
		else 
			return true;
